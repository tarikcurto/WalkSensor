...
void mpu6050_init()
{
    uint8_t buf0[2] = {0x6B, 0x80}; // PWR_MGMT_1 RESET
    i2c_write_blocking(i2c_default, addr, buf0, 2, false);
    sleep_ms(100);

    uint8_t buf1[] = {0x68, 0x07}; // SIGNAL_PATH_RESET:  GYRO_RESET | ACCEL_RESET | TEMP_RESET
    i2c_write_blocking(i2c_default, addr, buf1, 2, false);
    sleep_ms(100);

    uint8_t buf2[] = {0x6B, 0x00}; // PWR_MGMT_1 CLKSEL = 0 (INTERNAL 8MHz)
    i2c_write_blocking(i2c_default, addr, buf2, 2, false);
    sleep_ms(100);

    uint8_t buf3[] = {0x1B, 0x18}; // GYRO_CONFIG FS_SEL = 3
    i2c_write_blocking(i2c_default, addr, buf3, 2, false);
    sleep_ms(100);

    // uint8_t buf4[] = {0x1C, 0x16}; // ACCEL_CONFIG AFS_SEL = 3
...
void mpu6050_read(float accel[3], float gyro[3], float *temp)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        accel[i] = raw_accel[i] / accel_div;
        gyro[i] = raw_gyro[i] / gyro_div;
    } 
